...
app.get("/candidates/:id", (req, res) => {
  ...
  let keywords;
  p = p.then(() =>
    voiceOfSupporter(req.params.id).then(rows => {
      keywords = rows;
    })
  );
...
app.get("/political_parties/:name", (req, res) => {
  ...
  p = p.then(() =>
    pool
      .query("SELECT * FROM candidates WHERE political_party = ?", [
        req.params.name
      ])
      .then(rows => {
        candidates = rows;
...
app.post("/vote", (req, res) => {
  ...
  p = p.then(() =>
    pool
      .query(
        ...
        [req.body.name, req.body.address, req.body.mynumber]
      )
      .then(rows => {
        user = rows[0];
      })
  ...
  p = p.then(() =>
    pool
      .query("SELECT * FROM candidates WHERE name = ?", [req.body.candidate])
      .then(rows => {
        candidate = rows[0];
      })
  );
  ...
  let candidates = {};
  p = p.then(() =>
    pool.query("SELECT * FROM candidates").then(rows => {
      candidates = rows;
    })
  );
